...
**ユーザーの指示を待たずに** 以下のコマンドを実行して Snowflake Workspace にアップロードすること。
...
### スクリプトの仕様
- `.env` から Snowflake 接続情報を自動読み込み（`SNOWFLAKE_ORGANIZATION_NAME` / `SNOWFLAKE_ACCOUNT_NAME` / `SNOWFLAKE_USER` / `SNOWFLAKE_PASSWORD`）
- 指定ディレクトリ内の全 `.ipynb` を `snow://workspace/USER$.PUBLIC.DEFAULT$/versions/live/` にアップロード
- 確認場所: Snowsight > Projects > Workspaces

### 対象外
- `.ipynb` の編集が軽微な修正（コメント・Markdown セルのみ）の場合は任意
- ユーザーが「アップロード不要」と明示した場合はスキップ
