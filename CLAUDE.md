...
`experiments/` 配下の `.ipynb` は **Snowflake Notebooks** で実行する前提で作成すること。

### セルタイプの指定
- **SQL を書くセルは必ず SQLセル** にすること。Pythonセルに SQL を書いても Snowflake Notebooks では実行できない。
- セルタイプは `.ipynb` の JSON 内 `metadata` で以下のように指定する：

```json
// SQL セル
"metadata": { "snowflake": { "type": "sql" } }

// Python セル
"metadata": { "snowflake": { "type": "python" } }
```

- `NotebookEdit` ツールでセルを作成・編集する際は、`cell_type` を `"code"` にした上で、上記 `metadata` を必ず付与すること。
